classdef sar_adc_core < handle
    ...
    %   - convertInstantFast / convertVectorFast 是高吞吐版本，用于 128 路时间交织模数转换器或长串行链路
    %     波形等大规模仿真。快速版本跳过诊断警告和跟踪记录，默认参数已由设置函数校验，
    %     输出顺序仍与带检查版本一致。
    ...
    methods
        ...
        function [Dout_dec, Vout, Dout] = convertInstantFast(obj, Vin)
            %CONVERTINSTANTFAST 面向大规模仿真的单点快速 SAR 转换。
            %   该路径跳过输入过驱警告、Vdac 范围警告和跟踪记录。
            %   需要诊断信息和可选跟踪信息时使用 convertInstant。
            %   本函数的核心判决逻辑应与 convertVectorFast 保持一致。
            VL_local = obj.VL;
            VH_local = obj.VH;
            N_local = obj.N;
            Gain_local = obj.Gain;
            ...
            Vout = VL_local + Dout_dec * outScale_local;
        end
