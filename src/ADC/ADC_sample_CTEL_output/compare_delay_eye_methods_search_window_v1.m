...
%}
function compare_delay_eye_methods_search_window_v1()
%COMPARE_DELAY_EYE_METHODS_SEARCH_WINDOW_V1 Compare threshold-based and windowed cross-correlation-based CTLE eye diagrams.
...
% 搜索窗口 V1：先用阈值法得到粗延时，再只在粗延时本附近做互相关峰值搜索。
xcorrSearchWindowSymbolCount = 20;
...
[xcorrDelayIndex, xcorrPeakScore, xcorrSearchStartIndex, xcorrSearchStopIndex, xcorrGlobalDelayIndex, xcorrGlobalPeakScore] = findWindowedXcorrDelayIndex(txEffectiveVoltage, ctleVoltage, xcorrReferenceSampleCount, thresholdDelayIndex - txDelayIndex, xcorrSearchWindowSymbolCount * oversample);
...
fprintf('Xcorr search window symbol count: %d UI\n', xcorrSearchWindowSymbolCount);
fprintf('Xcorr search start index: %d\n', xcorrSearchStartIndex);
fprintf('Xcorr search stop index: %d\n', xcorrSearchStopIndex);
fprintf('Xcorr global delay index: %d\n', xcorrGlobalDelayIndex);
fprintf('Xcorr global peak normalized score: %.12g\n', xcorrGlobalPeakScore);
...
function [delayIndex, peakScore, normalizedScore] = findXcorrDelayIndex(txVoltage, rxVoltage, referenceSampleCount)
...
function [delayIndex, peakScore, searchStartIndex, searchStopIndex, globalDelayIndex, globalPeakScore] = findWindowedXcorrDelayIndex(txVoltage, rxVoltage, referenceSampleCount, coarseDelayIndex, searchHalfWindowSampleCount)
%FINDWINDOWEDXCORRDELAYINDEX 在阈值法粗延时附近的窗口内寻戾互相关峰值。
% 同时保留全局互相关峰值，便于对比窗口限制前后的差异。

[globalDelayIndex, globalPeakScore, normalizedScore] = findXcorrDelayIndex(txVoltage, rxVoltage, referenceSampleCount);
scoreCount = numel(normalizedScore);
searchStartIndex = max(1, coarseDelayIndex - searchHalfWindowSampleCount);
searchStopIndex = min(scoreCount, coarseDelayIndex + searchHalfWindowSampleCount);

if searchStartIndex > searchStopIndex
    error('Invalid cross-correlation search window.');
end

[peakScore, localPeakOffset] = max(normalizedScore(searchStartIndex:searchStopIndex));
delayIndex = searchStartIndex + localPeakOffset - 1;
end
