classdef sar_adc_metrics
    ...
    methods (Static)
        ...
        function [DNLmax, DNLmin, INLmax, INLmin] = ramp_INLDNL(Dout, N, plot_en)
            ...
            min_bin = min(Dout);
            max_bin = max(Dout);
            edges = (min_bin - 0.5):(max_bin + 0.5);
            h = histcounts(Dout, edges);
            mean_l = length(Dout) / 2^N;
            hdnl = h / mean_l - 1;
            point = 0;
            dnl = hdnl(1+point:end-point);
            inl = cumsum(dnl);
            DNLmax = max(dnl);
            DNLmin = min(dnl);
            INLmax = max(inl);
            ...
            if plot_en
                figure;
                subplot(2, 1, 1);
                Q_DNL = plot(linspace(min_bin+point, max_bin-point, length(dnl)), dnl, 'k');
                hold on;
                set(gca, 'linewidth', 2);
                ...
                subplot(2, 1, 2);
                Q_INL = plot(linspace(min_bin+point, max_bin-point, length(dnl)), inl, 'k');
                hold on;
                set(gca, 'linewidth', 2);
                set(gca, 'FontWeight', 'bold', 'fontsize', 15, 'fontname', 'Arial');
